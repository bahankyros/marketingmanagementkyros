...
## Role
This agent owns backend-facing React logic, Supabase reads and writes, CSV ingestion, derived financial calculations, upload state, and reusable data hooks. The mission is to keep data flows secure, outlet-aware, race-safe, and easy to audit.

## Core Rules
- Treat Supabase schema, RLS, RPCs, and Storage policies as the backend contract.
- Use the configured Supabase client, typed payloads, and RPCs where they match the schema.
- Prefer Supabase Realtime channels for live operational views when the screen benefits from real-time updates.
- Always unsubscribe listeners on cleanup.
- Never start a user-scoped fetch until auth state is ready and the app user profile exists.
...
2. Any supervisor query against outlet-bound data must include an `.eq('outlet_id', userData.outlet_id)` filter.
...
## CSV Import Rules
- Parse CSV files into a staging structure first. Never write raw parsed rows directly to Supabase.
...
- Use deterministic row identity for re-imports so the same month can be safely replaced.
- Store upload metadata such as `source_file_name`, `source_batch_id`, actor user ID, and timestamps for auditability.
...
- Keep upload state separate from database row state.
...
- Never store temporary browser object URLs in Supabase.
...
- Text proof and image proof must be built into one named payload object before the Supabase update.

## Real-Time Listener Rules
- Use secure Supabase Realtime listeners only for screens that truly need live state.
...
## Write Safety Rules
- Build payloads in a named object before calling `insert`, `update`, or `upsert`.
- Strip or block any field that can resolve to `undefined`.
- Use the application user profile ID for actor identifiers.
...
- Put fetching, synthesis, aggregation, CSV transformation, and Supabase subscription logic into decoupled hooks or helpers.
...
- Starting queries before auth, `role`, or `outlet_id` exists.
- Writing derived budget values from ad hoc inline math in UI handlers.
- Writing proof fields before Storage upload finishes.
- Passing raw database rows deep into UI trees when normalized data would be safer.
...
- Upload state cannot leak incomplete image data into Supabase.
